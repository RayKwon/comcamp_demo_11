...
App.Router.map(function(){
	this.resource('posts', function(){	// http://localhost/#/posts
		this.resource('post', { path: '/:post_id'}, function(){// http://localhost/#/posts/111
			this.route('edit');	// http://localhost/#/posts/111/edit
		});	
	});	
});
...
App.PostRoute = Ember.Route.extend({
	model: function(params){
		...
		var post = posts.findBy('id', parseInt(params.post_id));
		return post;
	}
});
